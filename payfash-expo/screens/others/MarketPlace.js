...
export default function PlansScreen({ navigation }) {
  ...
  const subscribePlan = async (planId) => {
    try {
      const response = await api.post("/plan/plan/souscrire", { planId }); // route backend: /plan + /plan/souscrire
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        {plans.map((plan) => (
          <TouchableOpacity
            key={plan.id}
            style={[styles.card, { borderColor: plan.color || "#4F46E5" }, currentPlan === plan.id && styles.activeCard]}
            onPress={() =>
...
const styles = StyleSheet.create({
  ...
  current: { marginTop: 10, color: "#4F46E5", fontWeight: "bold" },
});
